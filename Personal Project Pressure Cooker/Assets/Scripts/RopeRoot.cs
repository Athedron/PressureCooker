...
public class RopeRoot : MonoBehaviour
{
    ...
    private void AddChildren(Transform parent)
    {
        for (int i = 0; i < parent.childCount; i++)
        {
            ...
            {
                representative.GetComponent<Rigidbody>().freezeRotation = false;
                representative.GetComponent<SphereCollider>().isTrigger = true;
                //representative.gameObject.AddComponent<AttachBoxToRope>();
            }
        }
    }
